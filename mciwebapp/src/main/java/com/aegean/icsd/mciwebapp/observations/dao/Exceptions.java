...
final class Exceptions {
  ...
  static MciException FailedToRetrievePaths(String id, OntologyException e) {
    return new MciException(CODE_NAME + "." + 4, String.format("There was a problem when retrieving the images associated to the " +
      "id %s.",id), e);
  }

  static MciException FailedToRetrieveObservationItems(String id, Throwable e) {
    return new MciException(CODE_NAME + "." + 6, String.format("Could not retrieve the items of game with id %s ", id), e);
  }

  static MciException FailedToAskTheSolution(String id, String player, String word, Integer occurrences, Throwable e) {
